...
Change the summit categories, font size and mostly anything else using a text editor and file `BitmapMaps.ini`. A template will be made and placed in your home directory.

You can manually edit intermediate images like 'Markers.png', 'Grid.png' or 'Lakes.png', or text files like 'Summits.csv'. They are all generated by running 'pipeline'. After making changes, just re-run the pipeline and your changes will be included in the result, 'Composite.svg' (which you would print). Each sheet is stored in a separate folder, but the file names are the same.
...
Summits are marked in text and in 'Markers.png'. Summit candidates are identified from [surface curvature](https://en.wikipedia.org/wiki/Curvature#Curves_on_surfaces) and filtered out by [summit prominence](https://en.wikipedia.org/wiki/Topographic_prominence). Surface curvature helps filter out trees, masts and other local maxima. Prominence considers all the sheets included in a regional map, so a final value is found after making preliminary versions of all the sheets . For candidates with sufficently high prominece, we fetch names from `Stadnamn.jl` (you can make corrections in 'Stadnamn.ini' in your home folder).

Ridges ('outside corners', clifftop edges) and dieders ('inside corners', creeks, cliff bottom edges) are marked in `ridges.png` as dark or blue lines. They are found from principal curvature components and the thresholds are of course configureable.

The entire regional map is shown in a mosaic of sheets, zoomed out so as to also fit on a printed sheet. Details are removed, so it looks more like this image:
...
After downloading relevant .tif file, the work is done step-by-step in a 'pipeline'. Each step produces files, and re-running the pipeline will re-use such files unless you delete them. If you edit intermediate files, re-running the pipeline will pick those changes up and incorporate them in later steps.

The end result is one vector (.svg) image file per sheet, and an overview .svg for navigation on-screen. Although the intended end purpose is printing on paper, you can open the .svg files in an editor. The overview or regional map is linked to the sheets, you can inspect individual sheet files as they are being made.

Rendering the finished bitmaps is not realistically expected to run in one step. This is more likely:

- Step 1: `run_bitmapmap_pipeline()`. If not existing already, a default `BitmapMaps.ini` file is created in your home directory. User defines the map by updating arguments in that file (recommended) or by passing keywords to the pipeline. Keyword arguments will over-rule those in the .ini file.
- Step 2: User moves relevant elevation data into user's project folder. It may cover a larger area. Update BitmapMaps.ini file with this location.
- Step 3: `run_bitmapmap_pipeline()` again. If the input files are found, subfolders will be generated.
- Step 4: Enjoy. Or print to paper and enjoy.

Steps in the pipeline are :

Pre-process: Make an .svg file which links to each sheet, even though the linked files are not yet created (`make_vector_graphics`).
...
12) Find the prominence of summits and draw markers (`summits_on_sheet`). 
    Write an .csv file with summit position, elevation and curvature.
13) Make a composite bitmap from layers, and a simplified thumbnail bitmap (`join_layers`).
14) Shrink a shrunken composite bitmap (`make_thumbnail`) without grids and contours. See `Pre-process` above.
15) (Run only after 1-14 is completed for all sheets): 
    Calculate prominence (`summits_regional_update`), which updates files from step 12 with prominence and summit names.
16) Call `make_vector_graphics`, but this time for individual sheets instead of the regional map. 
17) Re-make the composite bitmap (`join_layers`) because `Markers.png` was updated with some fewer summit marks in step 15. 
