...
"""
    ...
    water_overlay(fofo, cell_iter, cell2utm, f_I_to_utm, details_I)
    ---> Bool
...
The water body recognition algorithm tries to account for artifacts, but several parameters may need
adjusting and knowing which may benefit from some diagnostic details. A few details on that coordinate
will be printed if it exists within the sheet.
"""
function water_overlay(sb::SheetBuilder; details_utm = (0,0), details_I = CartesianIndex(0, 0))
    details_utm !== (0,0) && details_I !== CartesianIndex(0, 0) && throw(ArgumentError("Only one feedback argument can be nonzero."))
    # Feedback preparation
    if details_utm !== (0,0)
        details_I = CartesianIndex(func_utm_to_cell_index(sb)(details_utm))
        Δ = sb.f_I_to_utm(details_I) .- details_utm
        if hypot(Δ...) > 2
            @warn "details_utm = $details_utm is outside of $(bbox_external_string(sb)). Ignored."
            details_utm == (0,0)
            details_I = CartesianIndex(0, 0)
        end
    end
    # Go ahead
    water_overlay(full_folder_path(sb), sb.cell_iter, cell_to_utm_factor(sb), sb.f_I_to_utm, details_I)
end
function water_overlay(fofo, cell_iter, cell2utm, f_I_to_utm, details_I)
    # Early exits
    ...
    if isfile(joinpath(fofo, WATER_FNAM)) && isfile(ffnam_csv_lakes)
        # We anticipate that minor changes be made to CONSOLIDATED_FNAM,
        # mostly fixing bad data and artifacts concerning lakes.
        # User will rerun the pipeline to incorporate changes.
        if any(mtime(joinpath(fofo, CONSOLIDATED_FNAM)) .< [mtime(joinpath(fofo, WATER_FNAM)), mtime(ffnam_csv_lakes)])
            @debug "    $WATER_FNAM and $(splitpath(ffnam_csv_lakes)[end]) in $fofo\n           already exists and are newer than $CONSOLIDATED_FNAM. Exiting `water_overlay`"
            return true
        else
            @debug "    $WATER_FNAM and $(splitpath(ffnam_csv_lakes)[end]) in $fofo\n           are older than $CONSOLIDATED_FNAM. Continue to write over."
        end
    end
    # Harvest defaults.
    ...
    water_overlay(fofo, cell_iter, cell2utm, f_I_to_utm,
        ...
        r_art, artifact_lim, flux_lim, details_I)
    true
end
...
function water_candidate_segments(cell2utm, z, ∇²z,
    ...
    is_water_candidate  = is_local_and_bbox_diagonal_slope_in_range(cell2utm, z, slopes_local_lim, r_slopes, Δzlim, diagonal_length_lim, details_I) .|
...
function is_local_and_bbox_diagonal_slope_in_range(cell2utm, z, slopes_local_lim, r_slopes, Δzlim, diagonal_length_lim, details_I)
    # We fill sections with unacceptable local steepness with this:
    no_water_value = Float32(2 * slopes_local_lim)
    # Segmented image, downsampled from full resolution by cell2utm
    segments = locally_acceptable_steepness_segments(cell2utm, z, no_water_value, slopes_local_lim, r_slopes, details_I)
    # Feedback
    display_if_vscode(segments; randomcolor=true)
    ...
    # Segment no => bounding box
    bb_dic = bbox_internal_indices_dictionary(segments)
    # Dictionary for each segment's bounding box diagonal (kind of like the enclosing diameter, but
    # dependant  on coordinate system orientation)
    bb_diag_len_dic = bbox_diagonal_length_dictionary(segments, bb_dic)
    dic_segno_details = Dict{Int64, Bool}(segno => true for (segno, bbx) in bb_dic if details_I ∈ bbx && axes(bbx) !== axes(labels_map(segments)))
    if ! isempty(dic_segno_details)
        printstyled("Segments with bounding box enclosing 'details_I' = $details_I: \n", color = :176)
        for segno in keys(dic_segno_details)
            if axes(bb_dic[segno]) !== axes(labels_map(segments))
                @show segno, bb_diag_len_dic[segno] * cell2utm, bb_dic[segno]
            end
        end
    end
    # Captures dictionaries and details_I
    function is_bbox_diagonal_slope_in_range(i)
        maxz = max_z_dic[i]
        minz = min_z_dic[i]
        Δz = maxz - minz
        l = cell2utm * bb_diag_len_dic[i]
        # Nominal length
        l_nom = min(l, diagonal_length_lim)
        # Nominal length fraction (0..1)
        u = l_nom / diagonal_length_lim
        if get(dic_segno_details, i, false)
            if Δz <= u * Δzlim
                printstyled("In range, details: ", color = :green)
            else
                printstyled("Not in slope range, details: ", color = :red)
            end
            @show i, minz, maxz, l, l_nom, Δzlim
            @show u * Δzlim, Δz
        end
        # Is this segment within z variation for its diagonal size?
        Δz <= u * Δzlim
    end
    # Dictionary of approved segments
    dic_bool = Dict{Int64, Bool}(i => is_local_flat(i) && is_bbox_diagonal_slope_in_range(i) for i in segment_labels(segments))
...
function locally_acceptable_steepness_segments(cell2utm, z, no_water_value, slopes_local_lim, r_slopes, details_I)
    # Where steepness is above the slopes_local_lim, we set
    # the value to 'no_water_value', for easy segmentation below.
    st = downsample(local_steepness_where_acceptable(z, no_water_value, slopes_local_lim, r_slopes), cell2utm)
    if ! iszero(details_I)
        steepness_at_details = st[details_I]
        if steepness_at_details == no_water_value
            printstyled("Local steepness at details_I indicates no water. \n", color = :176)
        else
            printstyled("Local steepness at details_I is $(steepness_at_details) m/m at elevation $(z[cell2utm * details_I]) m\n", color = :176)
        end
    end
    k = 4 # Reduced from 6 based on utm (25140, 6936305) Aursnesvatnet, this allows more bigger segments.
    ...
    felzenszwalb(st, k, preliminary_minimum_pixels_count)
end
